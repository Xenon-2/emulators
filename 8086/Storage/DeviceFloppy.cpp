...
namespace fdc
{
	...
	// write sector to disk
		// 👇 INSERISCI QUESTO BLOCCO DA QUI IN POI
			if (m_currDrive < 4 && m_images[m_currDrive].loaded)
			{
				// Recupera il path del file reale registrato dentro l'immagine del floppy
				std::string realPath = m_images[m_currDrive].path.string();

				if (!realPath.empty())
				{
					// Richiama la funzione nativa dell'emulatore per salvare istantaneamente
					SaveDiskImage(m_currDrive, realPath.c_str());
				}
			}
			// 👆 FINE DELLA MODIFICA PER IL FLOPPY
